...
<%--             <c:forEach var="i" begin="0" end="${fn:length(refinedData.data[0])-1}">
   ...
   <!-- 재구조화 타입 설정 및 테이블 출력 -->
   <div>
      <input type="submit" id="melt" value="melt" /> 
      <input type="submit" id="cast" value="cast" />
      <input type="submit" id="split" value="split" />
   </div>
...
<%--       <c:forEach var="i" begin="0" end="${fn:length(restructuredData.data[0])-1}">
   ...
   <!-- 테이블 조인 & 리셋 -->
   <div>
   <a href="<c:url value="/analytics/list"/>" id="join" title="join 데이터 선택" >JOIN</a>
   <input type="reset"   id="reset" value="reset"/>
