...
public class AnalyticsService implements IAnalyticsService {
	...
	public ArrayList<SampleVO> analyticsDatabase(int fileId) {
		...
		try {
			...
			REXP rdata = rEngine
					.eval("(data <- read.table(text = data, sep =\",\", header = TRUE, stringsAsFactors = FALSE))");
	...
	public Map<String, Object> analyticsDatabaseInfo(int fileId) {
		...
		try {
			...
			// R에서 "data"라는 변수를 사용하기 위해 exp를 전역변수형태로 전달하는 메서드(0을 사용하면 전역변수로
			// 지정가능)
			rEngine.rniAssign("data", exp, 0);
			...
			// R스크립트를 변수로 선언하려면 데이터타입을 REXP로 해야한다. 스크립트 안의 내용은 R의 data변수를 읽어서
			// dataFrame이라는 변수로 지정한 후 JAVA의 rdataFrame 변수로 선언. ()를 통해 실행까지 시킴
			REXP rdataFrame = rEngine.eval(
					"(dataFrame <- read.table(text=data, sep=\",\", fill=TRUE, header=TRUE, stringsAsFactors=FALSE, skipNul=TRUE, encoding=\"UTF-8\", fileEncoding=\"UTF-8\"))");
			...
			for (int i = 0; i < keys.length; i++) {
				...
				switch (rdataList.at(i).rtype) {
				// rtype 은
				// https://github.com/s-u/rJava/blob/master/jri/REXP.java 참고
	...
	public Map<String, Object> ynTo10(String column) {
		...
		rEngine.eval("yn_to_10 <- function(row){" + "if(row==\"Y\") " + "row=1 " + "else " + "if(row==\"N\") "
				+ "row=0 " + "else " + "row=\"\"}");
	...
	public Map<String, Object> getFirstChrToNum(String column) {
		...
		rEngine.eval("firstchr_to_no <- function(row){" + "row=substr(row, 1, 1) " + "if(row==\"\")" + " return(0)"
				+ " else" + " return(as.integer(row))}");
	...
	public Map<String, Object> nullToN(String column) {
		...
		rEngine.eval("nullstring_to_N <- function(row){" + "if(row==\"N\")" + " row=\"N\"" + " else if(row==\"Y\")"
				+ " row=\"Y\"" + " else" + " row=\"N\"}");
	...
	public Map<String, Object> zeroToMeanByGroup(String groupcol, String datacol) {
		...
		// datacol을 groupcol에 따라 그룹화하고 평균을 구함 (소득열을 직업군으로 분류하고 직업별 평균 소득을 구하는
		// 함수)
		rEngine.eval("mean_by_group<-round(tapply(data$" + datacol + ", data$" + groupcol + ", mean))");

		// 데이터가 0인 행을 자신이 속한 그룹의 평균 값으로 수정하는 함수 (소득이 0인 사람의 데이터를 직업별 평균 소득으로
		// 수정하는 함수)
		rEngine.eval("zero_to_mean<-function(occp, incm) {" + "if(incm==0)" + " return(mean_by_group[occp+1])" + " else"
				+ " return(incm)}");
	...
	public Map<String, Object> getRestructuredData() {
		...
		rEngine.eval(
				...
				"install.packages(\"reshape\"); " + "library(resape); " + "data<-as.data.frame(data); "
	...
	public void getMergedTable(int fileId1, int fileId2) {
		...
		try {
			...
			rEngine.eval("Sys.setlocale(category=\"LC_ALL\", locale=\"English_United States.1252\")");
			rEngine.eval(
					"data2 <- read.table(text=data2, sep=\",\", fill=TRUE, header=TRUE, stringsAsFactors=FALSE, skipNul=TRUE, encoding=\"UTF-8\", fileEncoding=\"UTF-8\"))");
	...
	public void xgboost(String column) {
		...
		rEngine.eval("install.packages('xgboost'); " + "library(xgboost) " + "data <- cbind(subset(data, select=-"
	 ...
	 * // 연령대 별 사기자 수!
	 * 
	 * @Override public ArrayList<RTestVO> getCountByAgeGen() {
	 * ArrayList<RTestVO> ageGenList = new ArrayList<RTestVO>(); try { REXP
	 * result1 = rEngine.
	 * eval("(count_by_age_gen <- table(subset(data_cust, select=AGE, subset=(data_cust$SIU_CUST_YN==1))))"
	 * ); REXP result2 = rEngine.
	 * eval("names(count_by_age_gen) <- c(\"0대\",\"10대\",\"20대\",\"30대\",\"40대\",\"50대\",\"60대\",\"70대\")"
	 * ); REXP result3 =
	 * rEngine.eval("barplot(count_by_age_gen, main = \"연령대별 사기자 수\")");
	 * 
	 * } catch (Exception e) { logger.error(e.getMessage()); throw new
	 * RuntimeException(e); } return ageGenList; }
	 * 
	 * // 성별 별 사기자 수!
	 * 
	 * @Override public ArrayList<RTestVO> getCountBySex() { ArrayList<RTestVO>
	 * sexList = new ArrayList<RTestVO>(); try { REXP result1 = rEngine.
	 * eval("(count_by_sex <- table(subset(data_cust, select=SEX, subset=(data_cust$SIU_CUST_YN==1))))"
	 * ); REXP result2 =
	 * rEngine.eval("names(count_by_sex) <- c(\"남자\", \"여자\")"); REXP result3 =
	 * rEngine.
	 * eval("pie(count_by_sex, cex=0.8, main=\"성별별 사기자 수\", labels=paste(names(count_by_sex), \"\\n\", A. count_by_sex, \"명\", \"\\n\", B. round(count_by_sex/sum(count_by_sex)*100), \"%\"))"
	 * );
	 * 
	 * } catch (Exception e) { logger.error(e.getMessage()); throw new
	 * RuntimeException(e); } return sexList; }
	 */
